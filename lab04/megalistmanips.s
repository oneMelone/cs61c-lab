...
mapLoop:
    lw t1, 0(s0)        # (e add -> lw)load the address of the array of current node into t1
    lw t2, 4(s0)        # load the size of the node's array into t2

	slli t3, t0, 2		# (e) 4 bytes each element
	...
	lw t0, 0(sp)
	lw t1, 4(sp)
	lw t2, 8(sp)		# (e) store and load temparary regs around call
	addi sp, sp, 12	
    ...
    lw a0, 8(s0)        # (e la -> lw) load the address of the next node into a0
    mv a1, s1		    # (e lw -> mv) put the address of the function back into a1 to prepare for the recursion
